document.addEventListener("DOMContentLoaded", () => {
  ...
  const collaborativeProjects = document.getElementById(
    "collaborativeProjects"
  );
  const bgPattern = document.getElementById("bgPattern");
  const bgPatternDropdown = document.getElementById("bgPatternDropdown");
  ...
  const toggleProjectsVisibility = () => {
    ...
    filterButton.textContent = collaborativeProjects.classList.contains(
      "d-none"
    )
  ...
  navbarCollapse.addEventListener("show.bs.collapse", () => {
    ...
    // Only manipulate SVGs on mobile
    if (window.innerWidth < 992) {
      bgPattern.style.display = "none";
      bgPatternDropdown.style.clipPath = "none";
    }
  });
  ...
  navbarCollapse.addEventListener("hidden.bs.collapse", () => {
    ...
    if (window.innerWidth < 992) {
      bgPattern.style.removeProperty("display");
      bgPatternDropdown.style.clipPath = "";
    }
  });
});
